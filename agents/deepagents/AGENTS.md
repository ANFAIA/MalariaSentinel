...
### Two orchestrators
- **Onboarding** (`deepagents onboard`): read-only, YAML-menu-driven, hands off to improver.
- **Improvement** (`deepagents improve -g "..."`): edit-capable, goal-driven, uses registry + plugin chain.
...
### CLI
```
deepagents onboard                # interactive menu
deepagents improve -g "..."       # improver
deepagents improve -g "..." --plan docs/plans/in-process/m12.md
deepagents status                 # scorecards, plans, subagents
deepagents agents list            # all subagents
deepagents agents show abm        # one subagent's details
deepagents run -g "..."           # back-compat alias
```
