"""build_environment: orchestrate the M1.3a data loaders to produce the env tensor + habitat patches.

Promoted from mal-ghana-sim/src/mal_ghana_sim/scripts/build_env.py (M7).

Usage:
    uv run python -m mal_execution.scripts.build_environment \\
...
def _resolve_aoi(
    ...
    if aoi is None:
        raise argparse.ArgumentTypeError("either --aoi or --bbox is required")
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="AOI slug. Looks up the AOI in the in-repo registry (currently: ghana) "
             "or uses --bbox if both are given. Default: ghana.",
    )
    parser.add_argument(
        "--bbox", default=None,
        help='Optional bbox "W,S,E,N" in the CRS. If set, overrides the slug registry.',
    )
    parser.add_argument("--crs", default="EPSG:4326", help="CRS for the AOI.")
    parser.add_argument("--resolution-m", type=int, default=1000,
                        help="AOI ground resolution in metres.")
    parser.add_argument("--year", type=int, required=True, help="Year (e.g. 2024).")
    parser.add_argument("--month", type=int, required=True, choices=range(1, 13),
                        help="Month (1-12).")
    parser.add_argument("--output-dir", type=pathlib.Path, required=True,
                        help="Directory for the env COG, sidecar, and habitat gpkg.")
    parser.add_argument("--scale", default=Scale.REGIONAL.value,
                        choices=[s.value for s in Scale],
                        help="Multi-scale level (regional/national/continental).")
    parser.add_argument("--name", default=None,
                        help="Optional human-readable name for the AOI.")
    ...
    parser.add_argument("--skip-worldcover", action="store_true",
                        help="[deprecated] Alias for --skip-jrc-gsw. The water_frac channel "
                             "is now sourced from JRC GSW 30 m, not ESA WorldCover 10 m.")
    parser.add_argument("--format", dest="output_format", default="tif",
                        choices=["tif", "nc"],
...
def main(argv: list[str] | None = None) -> None:
    ...
    skip_water = bool(args.skip_jrc_gsw or args.skip_worldcover)

    aoi_obj = _resolve_aoi(args.aoi, args.bbox, args.crs, args.resolution_m, scale, args.name)
    args.output_dir.mkdir(parents=True, exist_ok=True)
    h, w = aoi_obj.cells_per_side()
    suffix = f"{aoi_obj.slug}_{aoi_obj.scale.value}_{args.year:04d}_{args.month:02d}"
    ext = "nc" if args.output_format == "nc" else "tif"
    env_path = args.output_dir / f"{suffix}_env.{ext}"
    habitat_path = args.output_dir / f"{suffix}_habitat_patches.gpkg"

    print(
        f"build_env: AOI={aoi_obj.slug} scale={aoi_obj.scale.value} "
        f"grid={h}x{w} year={args.year} month={args.month:02d} format={args.output_format}"
    )

    water_frac: xr.DataArray
    if skip_water:
        print("skip-jrc-gsw set: filling water_frac with NoData (-9999.0).", file=__import__("sys").stderr)
        water_frac = _empty_channel(aoi_obj, value=NODATA_SENTINEL, band_name="water_frac")
    else:
        water_frac = _load_with_fallback(
            load_jrc_gsw_water_frac, aoi_obj, 2021, args.month, "water_frac",
        )

    if args.output_format == "nc":
        # Daily NetCDF: use daily loaders
        if args.skip_era5:
            print("skip-era5 set: filling water_temp_c with NoData (-9999.0).", file=__import__("sys").stderr)
            h_, w_ = aoi_obj.cells_per_side()
            import calendar as _cal
            n_days = _cal.monthrange(args.year, args.month)[1]
            ...
            water_temp_c = _load_with_fallback(
                load_era5_water_temp, aoi_obj, args.year, args.month, "water_temp_c",
            )

        rainfall = _load_with_fallback(
            load_chirps_rainfall_daily, aoi_obj, args.year, args.month, "rainfall",
        )

        if args.skip_modis:
            print("skip-modis set: filling ndvi with NoData (-9999.0).", file=__import__("sys").stderr)
            ndvi = _empty_channel(aoi_obj, value=NODATA_SENTINEL, band_name="ndvi")
        else:
            ndvi = _load_with_fallback(
                load_modis_ndvi, aoi_obj, args.year, args.month, "ndvi",
            )

        _write_env_nc(
            env_path, rainfall, water_temp_c, water_frac, ndvi,
            aoi_obj, args.year, args.month,
        )
        print(f"wrote env NC    -> {env_path}")
    else:
        # COG/TIF: existing behavior
        if args.skip_era5:
            print("skip-era5 set: filling temp_suitability with NoData (-9999.0).", file=__import__("sys").stderr)
            ...
            temp_suitability = _load_with_fallback(
                load_era5_temp_suitability, aoi_obj, args.year, args.month, "temp_suitability",
            )

        rainfall = _load_with_fallback(
            load_chirps_rainfall, aoi_obj, args.year, args.month, "rainfall",
        )
        ...
        _write_env_cog(env_path, env, aoi_obj, args.year, args.month)
        print(f"wrote env COG   -> {env_path}  ({(4 * h * w * 4) / (1024 ** 2):.2f} MB)")

    dem = _load_with_fallback(load_merit_dem, aoi_obj, args.year, args.month, "elevation")

    _write_habitat_patches_gpkg(
        habitat_path, dem, water_frac, aoi_obj, twi_threshold=args.twi_threshold,
    )
    print(f"wrote habitat   -> {habitat_path}")
