...
TEST(MalAbmFastDispersal, Determinism) {
    ...
    mal_abm_fast::DispOffset a = mal_abm_fast::offset_m(p, 0.0, 0.0, 300.0, 800.0);
    mal_abm_fast::Prng q(123);
    mal_abm_fast::DispOffset b = mal_abm_fast::offset_m(q, 0.0, 0.0, 300.0, 800.0);
...
TEST(MalAbmFastDispersal, OffsetIsZeroWhenPrngIsUnseededAndZero) {
    ...
    mal_abm_fast::DispOffset o = mal_abm_fast::offset_m(p, 0.0, 0.0, 300.0, 800.0);
    EXPECT_TRUE(std::isfinite(o.dlon));
    EXPECT_TRUE(std::isfinite(o.dlat));
}

TEST(MalAbmFastDispersal, CapAtMaxMeters) {
    // 10000 draws with sigma=300, max=800, lon=0, lat=0. The
    ...
    for (int i = 0; i < N; ++i) {
        ...
        EXPECT_LE(std::abs(dx_m), 800.0 + 1e-9) << "i=" << i;
        EXPECT_LE(std::abs(dy_m), 800.0 + 1e-9) << "i=" << i;
    ...
    // very close to 800 m (within numerical noise).
    EXPECT_NEAR(max_dx, 800.0, 3.0);
    EXPECT_NEAR(max_dy, 800.0, 3.0);
}
...
TEST(MalAbmFastDispersal, StddevNearSigmaMeters) {
    ...
    EXPECT_NEAR(mx, 0.0, 15.0);
    EXPECT_NEAR(my, 0.0, 15.0);
    EXPECT_NEAR(sdx, 300.0, 30.0);
    EXPECT_NEAR(sdy, 300.0, 30.0);
}
...
TEST(MalAbmFastDispersal, CosLatCorrection) {
    ...
    dlon_hi.reserve(N);
    for (int i = 0; i < N; ++i) {
        mal_abm_fast::DispOffset eq = mal_abm_fast::offset_m(p_equator, 0.0,  0.0, 300.0, 800.0);
        mal_abm_fast::DispOffset hi = mal_abm_fast::offset_m(p_high,    0.0, 60.0, 300.0, 800.0);
