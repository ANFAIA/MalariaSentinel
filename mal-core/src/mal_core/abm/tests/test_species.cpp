...
TEST(Salinity, ColuzziiToleratesMoreThanGambiaeSS) {
    ...
    // Both must be strictly positive (not melas-like hard-zero) at 35 psu,
    // but coluzzii strictly higher than gambiae s.s. at equal psu > opt.
    for (float psu = 5.0f; psu <= 45.0f; psu += 5.0f) {
    ...
    EXPECT_GT(salinity_suitability(col, 35.0f), 0.0f);
    EXPECT_GT(salinity_suitability(col, 35.0f),
              salinity_suitability(gam, 35.0f));
    // Low-but-positive tail at marine psu for coluzzii (nonzero, not full).
    const float s_sea = salinity_suitability(col, 35.0f);
    EXPECT_LT(s_sea, 0.35f);
    EXPECT_GT(s_sea, 0.0f);
}

TEST(Salinity, GamiaeSSNegligibleAtSeawater) {
    const auto gam =
        species_params_for(MosquitoSpeciesId::ANOPHELES_GAMBIAE_SS);
    // Freshwater species: essentially 0 at 35 psu (strictly positive but small).
    EXPECT_LT(salinity_suitability(gam, 35.0f), 0.1f);
}
