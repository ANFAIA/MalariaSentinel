...
def load_chirps_rainfall_daily(
    ...
    *,
    ...
    Always returns a 3-D (time, y, x) DataArray, even for a single month.
    ...
    Returns:
        ...
        Values in mm/day. Time coordinate uses ``cftime.DatetimeGregorian``.
    """
    import cftime
    ...
    for year in sorted(years):
        ...
        for month in sorted(months):
            ...
            for d in days:
                ...
                time_coords.append(cftime.DatetimeGregorian(year, month, d))

    if not daily_arrays:
        raise ValueError("No data loaded — check years/months arguments")
